 ...
 * @LastEditTime: 2022-10-27 23:31:34
 */
...
const parseLi = (li, lvl = 0, result = [], nodeStructOptions) => {
  ...
  if (ou.length) {
    ...
    for (let i = 0; i < ou.length; i++) {
      ...
      if (end > start) {
        const p = paragraphParser({ ...li, [CHILDREN]: children.slice(start, end), [STYLE]: { ...style, 'mso-char-indent-count': `-${lvl + 1}.0`, 'mso-para-margin-left': `${lvl}.0gd` } }, nodeStructOptions);
    ...
    if (start < children.length) {
      const p = paragraphParser({ ...li, [CHILDREN]: children.slice(start), [STYLE]: { ...style, 'mso-char-indent-count': `-${lvl + 1}.0`, 'mso-para-margin-left': `${lvl}.0gd` } }, nodeStructOptions);
      if (p.length) result.push(p[0]);
    }
  } else {
    const p = paragraphParser({ ...li, [STYLE]: { ...style, 'mso-char-indent-count': `-${lvl + 1}.0`, 'mso-para-margin-left': `${lvl}.0gd` } }, nodeStructOptions);
    if (p.length) result.push(p[0]);
  }
};
// 处理有序列表或无序列表，直接按悬挂缩进2个字符的段落处理，不会获得任何样式
export const listParser = (node, nodeStructOptions) => {

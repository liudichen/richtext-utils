 ...
 * **/
export const htmlSpacingSizeToWordSizeNumber = (size: number | string, wordPtFontSize?: number) => {
  if (!size) return;
...
// 上 右 下 左
export const splitHtmlMarginString = (marginStr: string | number) => {
  if ((typeof marginStr !== 'string' && typeof marginStr !== 'number') || !marginStr) return [];
  if (typeof marginStr === 'number') marginStr = `${marginStr}`;
  const margin = marginStr.split(/[\s]+/);

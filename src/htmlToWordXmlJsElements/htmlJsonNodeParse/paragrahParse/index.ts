...
export const getParagraphParamsFromStyle = (styles: ObjectStyle = {}, onlyHans = true, styleCamelCase = false) => {
  ...
  if (keys.includes(alignStr)) {
    ...
    if (align === 'right' || align === 'center' || align === 'left') {
      data.align = align;
    } else if (align === 'justify' && (styles[styleCamelCase ? 'textAlignLast' : 'text-align-last'] === 'justify' || styles[styleCamelCase ? 'textJustify' : 'text-justify'] === 'distribute-all-lines')) {
...
export const paragraphHtmlJsonNodeParser = async (node: HtmlJsonNode, getImageStepTwoParamsFn?: GetImageStepTwoParamsFn, options?: HtmlJsonNodeParserOptions) => {

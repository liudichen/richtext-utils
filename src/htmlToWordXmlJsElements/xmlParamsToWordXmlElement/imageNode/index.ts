...
export const imageXmlParamsNodeToXmlElementObj = (imageNode: HtmlXmlParamsImageNode, config: XmlElementGenerationConfig = {}) => {
  let { rId, name, cx, cy } = imageNode;
  if (!cx || !cy || !rId) return;
  const { imageMaxC } = config || {};
  const maxC = Math.max(cx, cy);
  if (imageMaxC && imageMaxC < maxC) {
    cx = Math.round(cx * imageMaxC / maxC);
    cy = Math.round(cy * imageMaxC / maxC);
  }
                      ...
                      { type: 'element', name: 'pic:nvPicPr',
                        elements: [
                          { type: 'element', name: 'pic:cNvPr',
                            attributes: { id: `${rId}`, name: `${name}` }, elements: [],
                          },

...
/**生成xml文本节点(w:r) */
export const textXmlParamsNodeToXmlElementObj = (
  textNode: HtmlXmlParamsTextNode,
  ...
  const w_r: XmlNode = {
    ...
    elements: [
      ...
      },
      {
        type: "element",
        name: "w:t",
        attributes: !text || text.startsWith(" ") ? { "xml:space": "preserve" } : {},
  ...
  let w_rFonts: XmlNode = {
    ...
    name: "w:rFonts",
    attributes: {
      "w:hAnsi": defaultFontFamily,
      "w:ascii": defaultFontFamily,
      "w:eastAsia": defaultFontFamily,
      "w:hint": "eastAsia",
    },
    elements: [],
  };
  ...
  if (backgroundColor) {
    w_r.elements[0].elements.push({
      type: "element",
      name: "w:highlight",
      attributes: { "w:val": `${backgroundColor}` },
      elements: [],
    });
  }
  if (underline) {
